...
export default function ServerInformation() {
  ...
  const selectedServerID = useRef([]);
  // const [selectedServerID.current, setSelectedServerID] = useState([]);
  // const selectedServerID.current = [];
  ...
  useEffect(() => {
    console.log('selectedServerID.current', selectedServerID.current);
  ...
  const handleNodeClick = id => {
    console.log('selectedServerID.current1', selectedServerID.current);
    const index = selectedServerID.current.indexOf(id);
    console.log('id', id);
    console.log('index', index);
    if (index !== -1) {
      console.log('aaaaa');
      // selectedServerID.current.splice(index,1)
      // selectedServer.splice(index,1)
      const updateSelectedServerID = selectedServerID.current.filter(
        item => item !== id
      );
      const updateSelectedServer = selectedServer.filter(
        item => item.name !== id
      );
      selectedServerID.current = updateSelectedServerID;
      setSelectedServer(updateSelectedServer);
    } else {
      // selectedServerID.current.push(id)
      ...
      setSelectedServer(previousSelectedServer => [
        ...previousSelectedServer,
        { name: id },
      ]);
      // console.log([...selectedServerID.current, id])
      console.log('save');
    }
  ...
  const handleNodeClickTemp = () => {
    console.log(selectedServerID.current);
  };
    ...
    <Stack sx={{ width: '100%' }}>
      <Stack direction='row' justifyContent='space-between' spacing={2}>
        ...
        <Stack sx={{ width: '68%' }} direction='column'>
          <Button onClick={handleNodeClickTemp}>test</Button>
          <Button onClick={handleNodeClick}>test11</Button>
          <ClusterInfo data={selectedServer} />
        </Stack>
      </Stack>
    </Stack>
  );
