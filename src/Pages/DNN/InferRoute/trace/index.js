...
export default function InferRouteTrace() {
  //定义-开始
  //#region
  ...
  const { routeService, routeFailed } = useSelector(state => {
    return {
      routeService: state.InferRoute.routeService,
      routeFailed: state.InferRoute.routeFailed,
    };
  });
  ...
  const changePage = (id, start, end) => {
    navigate(`/detail/infertrace/${start}/${end}/${id}`);
  };
