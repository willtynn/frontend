...
export function PlanAlgorithmManage(props) {
    ...
    const { exe_alg_list } = useSelector(state => {
        return {
    ...
    const rows = _.map(exe_alg_list, function (obj) {
        //小于等于3的我们认为其实是系统默认算法，不允许修改和查看
        if (obj.plan_id <= 3) {
            return {
                id: obj.plan_id,
                name: obj.plan_name,
                description: obj.plan_text,
                content: obj.plan_content,
                type: "system"
            }
        } else {
            return {
    ...
    const handleModifyAlgorithm = () => {
        ...
        dispatch(exe_modify(modifyAlgorithm))
        dispatch(evo_get_algorithm());
        changeListState();
        setPage(0);
        setState("all")
    ...
    const handleGoDetail = (e, index) => {
        ...
        // const anaList = _.filter(ana_alg_list, function (obj) {
        //     if (obj.id == visibleRows[index].id)  {
        //       return obj;
        //     }
        //   })
