...
export function PlanAlgList() {
    ...
    const { pageSize, pageNum, evo_plans, exe_alg_list } = useSelector(state => {
        return {
    ...
    const visibleRows = useMemo(() => {
        //当前算法的总数是exe_alg_list中的长度
        setCount(exe_alg_list.length);
        //如果页码超过了最大页码就返回1
        if (pageSize * (pageNum - 1) > count) {
            dispatch({ type: UPDATE_TEST_PLAN_PAGE_NUM, data: 1 });
            return exe_alg_list.slice(
                0,
                pageSize
            );
        }
    ...
    }, [pageNum, pageSize, searchList, exe_alg_list]);
                                            ...
                                            >
                                                ...
                                                <Box
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        setTarget("plan");
                                                        dispatch({type:EVO_UPDATE_EVO_PLAN_ALG,data:row})
                                                        handleAlgorithmClick();
