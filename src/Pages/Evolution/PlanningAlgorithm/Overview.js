...
export default function PlanningAlgorithmComponent() {
    ...
    useEffect(() => {
        if (Array.isArray(algorithms)) {
            const formattedData = algorithms
                .filter(source => source.type === 'plan') // 过滤出type为plan的算法
                .map(source => ({
                id: source.id,
                name: source.name,
                info: source.info,
                            ...
                            tableData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                                <TableRow key={index}>
                                    ...
                                    <StyledTableBodyCell
                                        ...
                                        onClick={() => navigate(`/detail/planningAlgorithm/${row.name}`)} // 跳转到详情页面
                                    >
                                        {row.name}
                                    </StyledTableBodyCell>
