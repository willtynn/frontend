...
export default function GeneralInfo({ algorithmName }) {
    ...
    const intl = useIntl();
    const backText = intl.messages['evolution.analysisAlgorithm']
    ...
    const analysisAlgorithmDetail = useSelector(state =>
    ...
    useEffect(() => {
        ...
        if (!analysisAlgorithmDetail) {
            dispatch(fetchAllAlgorithms());
        }
    }, [algorithmName, analysisAlgorithmDetail, dispatch]);

    const handleRefreshInstances = () => {
        const algorithmId = analysisAlgorithmDetail?.id;
        if (algorithmId) {
    ...
    const handleDelete = () => {
        // 查找对应的算法并获取 id
        if (analysisAlgorithmDetail) {
            const { id } = analysisAlgorithmDetail;

            // 调用 deleteAlgorithm 并传递 id
            dispatch(deleteAlgorithm(id)).then(() => {
                navigate('/evolution/analysisAlgorithm'); // 删除后返回数据源信息页面
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <Tooltip
                        ...
                        title={analysisAlgorithmDetail ? analysisAlgorithmDetail.info: ''}
                        placement='bottom'
                    >
                        <Box
                            sx={{
                        ...
                        >
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.name : ''}
                        </Box>
                    </Tooltip>
                </Stack>
                <Stack
            ...
            >
                ...
                <Stack sx={{ margin: '6px 0px' }} direction='column' spacing={2}>
                    ...
                    <Stack direction='row' spacing={0.75}>
                        ...
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.info : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.type']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.type : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.input']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.input : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.output']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.output : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.parameter']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.parameter : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.url']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.url : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.createTime']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.createTime : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.updateTime']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.updateTime : ''}
                        </Box>
                    </Stack>
                    <Stack direction='row' spacing={0.75}>
                        <Box sx={labelStyle}>{intl.messages['evolution.isDelete']}</Box>
                        <Box sx={valueStyle}>
                            {analysisAlgorithmDetail ? analysisAlgorithmDetail.isDelete : ''}
                        </Box>
                    </Stack>
                </Stack>
            </Box>
            ...
            <EditAlgorithm
                ...
                algorithmToEdit={analysisAlgorithmDetail} // 传递现有算法数据源
            />
            ...
            <RunAlgorithmForm
                ...
                instanceToCreate={analysisAlgorithmDetail} // 传递现有算法数据源
