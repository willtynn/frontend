...
export default function GeneralInfo(props) {
  ...
  const items = [
    [<EditService />, intl.messages['scheme.editScheme'], () => {}],
      ...
      >
        ...
        >
          <Stack direction='row' spacing={1}>
            ...
            >
              {intl.messages['scheme.deployScheme']}
            </Box>
          </Stack>
        </KubeCancelButton>
        <Stack
        ...
        >
          ...
          <Tooltip
            ...
            title={currentScheme !== null ? currentScheme.name : ''}
            placement='bottom'
          >
            <Box
              sx={{
            ...
            >
              {currentScheme !== null ? currentScheme.name : ''}
            </Box>
          </Tooltip>
        </Stack>
        <Stack
      ...
      >
        ...
        <Stack sx={{ margin: '6px 0px' }} direction='column' spacing={1.5}>
          <Stack direction='row' spacing={0.75}>
            <Box sx={labelStyle}>{intl.messages['common.id']}</Box>
            <Box sx={valueStyle}>
              {currentScheme !== null ? currentScheme.id : ''}
            </Box>
          </Stack>
          <Stack direction='row' spacing={0.75}>
          ...
          <Stack direction='row' spacing={0.75}>
            <Box sx={labelStyle}>{intl.messages['common.namespace']}</Box>
            <Box sx={valueStyle}>
              {currentScheme !== null ? currentScheme.namespace : ''}
            </Box>
          </Stack>
          <Stack direction='row' spacing={0.75}>
            <Box sx={labelStyle}>{intl.messages['common.status']}</Box>
            <Box sx={valueStyle}>
              {currentScheme !== null ? currentScheme.status : ''}
            </Box>
          </Stack>
          <Stack direction='row' spacing={0.75}>
            <Box sx={labelStyle}>{intl.messages['common.time']}</Box>
            <Box sx={valueStyle}>
              {currentScheme !== null ? formatDatetimeString(currentScheme.time) : ''}
            </Box>
          </Stack>
        </Stack>
      </Box>
